...
bool GraphicsLayerAndroid::repaint()
{
    ...
    if (!gPaused && m_haveContents && m_needsRepaint && !m_image) {
        ...
        if (m_foregroundLayer) {
            ...
            //Patch for MPSG5094:ContentRect should not go less than clipSize or ScrollingLayer Size ,otherwise ScrollLimits for the layer will be set to incorrect values,which will cause incorrect layer position on every scroll
