...
RenderLayer::RenderLayer(RenderBoxModelObject* renderer)
    ...
    , m_scrollCorner(0)
    , m_resizer(0)
    , m_iFrameInnerScroll(false) // SAMSUNG CHANGE
{
...
RenderLayer* RenderLayer::stackingContext() const
{
	...
	// SAMSUNG CHANGE +
    if (layer == NULL && this->m_iFrameInnerScroll) {
...
void RenderLayer::scrollRectToVisible(const IntRect& rect, bool scrollToAnchor, const ScrollAlignment& alignX, const ScrollAlignment& alignY)
{
    ...
    if (renderer()->hasOverflowClip() && !restrictedByLineClamp) {
		...
		// SAMSUNG CHANGE +
                ...
                int layerOffset = 0;
                if (frame && frame->document() && frame->document()->body() && frame->document()->body()->isContentEditable()) {
                    layerOffset = yOffset;
                    if (layerOffset > rect.y())
                        layerOffset = 0;
                }
		// SAMSUNG CHANGE -
                newRect.setX(rect.x() - frameView->scrollX() + frameView->x());
                newRect.setY(rect.y() - frameView->scrollY() + frameView->y() - layerOffset); // SAMSUNG CHANGE
            } else {
                IntRect viewRect = frameView->visibleContentRect();
                IntRect r = getRectToExpose(viewRect, rect, alignX, alignY);
