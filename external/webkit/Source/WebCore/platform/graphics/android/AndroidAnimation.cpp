...
void AndroidTransformAnimation::applyForProgress(LayerAndroid* layer, float progress)
{
    ...
    if (valid) {
        for (size_t i = 0; i < toValue->value()->size(); ++i){
			...
			//SAMSUNG CHNAGES >>
			//This changes is made for google image shaking issue during swipe.
		    const TranslateTransformOperation* toOp = static_cast<const TranslateTransformOperation*>(toValue->value()->at(i));
		    Length toX = toOp->x();
		    Length toY = toOp->y();
		    
		    const TranslateTransformOperation* fromOp = static_cast<const TranslateTransformOperation*>(fromValue->value()->at(i));
		    if(fromOp){
			    Length fromX = fromOp->x();
			    Length fromY = fromOp->y();
			        
			    const TransformationMatrix currenttTransformMatrix = layer->getTransform();
			    if(fromX.value() > toX.value()){
			    	if(currenttTransformMatrix.m41() < transformMatrix.m41())
			     		transformMatrix.setM41(currenttTransformMatrix.m41());
			    }else if( toX.value() > fromX.value() ){
			    	if(currenttTransformMatrix.m41() > transformMatrix.m41()  )
			     		transformMatrix.setM41(currenttTransformMatrix.m41());
			    }
			    
			    if(fromY.value() > toY.value()){
			    	if(currenttTransformMatrix.m42() < transformMatrix.m42())
			     		transformMatrix.setM42(currenttTransformMatrix.m42());
			    }else if( toY.value() > fromY.value() ){
			    	if(currenttTransformMatrix.m42() > transformMatrix.m42()  )
			     		transformMatrix.setM42(currenttTransformMatrix.m42());
			    }
			}
