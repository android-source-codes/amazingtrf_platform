                                             ...
                                             const FloatRect& clipping, float opacity, float scale)
{
	...
	XLOG("updateGLPositionsAndScale : [%d], haveClip(%d), clipping(%d,%d,%d,%d)", uniqueId(), m_haveClip, clipping.x(), clipping.y(), clipping.width(), clipping.height());
...
static inline bool compareLayerZ(const LayerAndroid* a, const LayerAndroid* b)
{
    ...
    if( a == NULL || b == NULL )
    {
          XLOGC("DIVYANK: compareLayerZ:SubLayers Null");
	  return false;
    }
    //SAMSUNG CHANGES <<
...
bool LayerAndroid::drawChildrenCanvas(SkCanvas* canvas)
{
    ...
    if (count > 0) {
        ...
        for (int i = 0; i < count; i++)
        {
            LayerAndroid* childLayer = this->getChild(i); //Samsung Changes
	    if(childLayer!=NULL)
	            sublayers.append(this->getChild(i));
        }
        // now we sort for the transparency
	...
	{          
		...
		XLOGC("DIVYANK: drawChildrenCanvas:SubLayers Null");
...
// SAMSUNG CHANGE+
LayerAndroid* LayerAndroid::findByOwner(RenderLayer* renderLayer)
{
    if (m_owningLayer == renderLayer)
        return this;
    for (int i = 0; i < countChildren(); i++) {
        LayerAndroid* result = getChild(i)->findByOwner(renderLayer);
        if (result)
            return result;
    }
    return 0;
